...
| `composio`              | Stub         | P1/EXPERIMENTAL candidate integration gateway (`integration.gateway`). Real vendor evaluated (`docs/skills/composio-integration.md`), no OAuth app/API key/live connection. HIGH risk: would hold per-tenant OAuth credentials for real external actions                                                 |
| `perplexity`            | Stub         | P2 optional research provider (`research.cited`), complementary to `scrapling`'s `web.research`, not a duplicate. Real vendor evaluated (`docs/skills/perplexity-research-integration.md`), no API key/live call. Never the default model                                                                |
