...
--
-- business_dna has real client policies (select/insert/update/delete, all
-- scoped to organization_id = auth_org_id()) -- covered here with the same
-- depth as calendar-booking-isolation.sql covers booking_types/
-- availability_rules. inbox_threads/inbox_messages/inbox_mailboxes are
...
begin
  ...
  if enabled_count <> 4 then
    raise exception 'INBOX-DNA RLS FAIL: expected RLS on 4 tables, found %', enabled_count;
  end if;
  ...
  if dna_policy_count <> 4 then
    raise exception 'INBOX-DNA RLS FAIL: expected 4 authenticated business_dna policies, found %', dna_policy_count;
  end if;
