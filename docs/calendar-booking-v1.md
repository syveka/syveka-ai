...
2. **Services** — `src/server/services/`
   ...
   - `booking.ts`: booking types (public pages), public slot computation, guest booking with **double-booking protection** (availability check → owner-calendar `pg_advisory_xact_lock` → re-check inside `$transaction` before insert - the lock is load-bearing: a bare re-check inside a transaction does _not_ prevent two concurrent guests from both committing under Postgres's default READ COMMITTED isolation, see `lockOwnerCalendarForBooking()`'s comment and `tests/integration/booking-concurrency.sh` for a real-Postgres proof), token-based cancel/reschedule (reschedule creates a linked successor booking).
